...
CREATE TABLE IF NOT EXISTS pre_linkscheer_category (
...
ALTER TABLE `pre_linkscheer_category`ADD `authority` TEXT   NOT NULL ;

CREATE TABLE `pre_linkscheer_authority` (
`id` INT( 11 ) NOT NULL AUTO_INCREMENT PRIMARY KEY ,
`authority` TEXT NOT NULL 
) ENGINE = MYISAM ;
ALTER TABLE `pre_linkscheer_question` ADD `is_auth` VARCHAR( 1 ) NULL ;
ALTER TABLE `pre_linkscheer_question` ADD `readpay` INT NULL ;

CREATE TABLE `pre_linkscheer_paylog` (
...
if($data){
  $data=linkscheer_flashToDz($data);
  $A=explode("\n",$data);
  $A[0]=$sql;
  $sql=implode("\n",$A);
}
