...
function tsound_wenda_content($T, $q = 1) {
	...
	if ($q) {
		if ($T ['uid'] == $_G ['uid'] && ! $A) {
			...
			$S ['buttons'] [] = '<button onclick="showWindow(\'tsound_wenda_other\', \'' . $baseurl . '&action=question_other&qid=' . $T ['qid'] . '\')" class="pn pnc"><strong>' . lang ( 'plugin/tsound_wenda', 'other_b' ) . '</strong></button>';
// 增加悬赏按钮 by TT 20140318			$S ['buttons'] [] = '<button onclick="showWindow(\'tsound_wenda_other\', \'' . $baseurl . '&action=coinadd&qid=' . $T ['qid'] . '\')" class="pn pnc"><strong>' . lang ( 'plugin/tsound_wenda', 'coinadd' ) . '</strong></button>';
