...
if(in_array($act,array('edit'))){
  if($act=='edit'){
    ...
    include_once(dirname(__FILE__).'/video.edit.php');
  }
  exit();
}
if(submitcheck('submit')){
	...
	showtableheader(lang('plugin/tsound', 'videolist'), 'nobottom');
	showtablerow('',
    array('', '', '', '','', '', '', '', '', '', '', ''),
    array(
      '',
  ...
  $count=DB::num_rows(DB::query("SELECT * FROM ".DB::table('tsound_record')." $xxx"));
	...
	$query = DB::query("SELECT r.*,m.username,t.subject as thread FROM ".DB::table('tsound_record')." r left join ".DB::table('common_member')." m on m.uid=r.uid left join  ".DB::table('forum_thread')." t on t.tid=r.tid where r.type='video' and r.status>=0 ORDER BY r.rid desc limit ".(($page - 1) * $perpage).','.$perpage);
	while($rs = DB::fetch($query)) {
		showtablerow('', '', array(
