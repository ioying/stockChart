...
if (!submitcheck('settingsubmit', 1)) {
	...
	showformheader('plugins&operation=config&do=' . $pluginid . '&identifier=tsound&pmod=video&act='.$act.'&id='.$id, 'enctype');
	
	showtableheader(lang('plugin/tsound', 'edit_video'));
  ...
  $onez['status']=intval($onez['status']);
	...
	cpmsg(lang('plugin/tsound', 'done_successfully'), 'action=plugins&operation=config&do=' . $pluginid . '&identifier=tsound&pmod=video', 'succeed');
}
?>
