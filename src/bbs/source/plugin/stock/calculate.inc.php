...
/**
 * calculate.inc.php 根据解析后的公式进行计算部分
 * @author  Ioying@hotmail.com  http://www.ttall.net http://www.ttlele.com
 * @copyright 2008
 * 
 * 20090427
 * 剥离原来写入 data.txt 的所有代码  ， 程序最后以 JSON 输出和传递。
 * 20140128 使用 getcsv.inc.php 获取行情数据,原 data.txt 废弃。
 *
 * 20090427
 * 以后使用数据库组件来完成数据库连接，这里暂时直接连接。
 * 而且以后可能使用其他方式存储数据，比如 文本 等。      行情数据使用文本，财务数据考虑使用数据库 by Ioying 20140222
 * 
 * 20090309 修改计划
 ...
 */
				...
				//	优化！ 将来要接受一些参数，比如 计算范围是从历史数据起点开始（重新计算）还是只计算新增部分 （刷新数据） 或从指定位置开始 200427
 
include 'func/make_str.func.php'; 
include 'func/not_a_func.func.php';
...
include 'getcsv.inc.php';  	// 从csv文件读取原始数据
				...
				/*改为读取 csv文件  ， getcsv.inc.php  ，原数据库连接和读取部分，转至getfromdatabase.php 20140127 */
...
foreach ($arrtest as $key => $value) {
    ...
    {
				//20090518  将原来$arrtest[$key][0] 改为 $arrtest[$key][0][0], 数组第一项数据由原来的行计算公式改为子数组，子数组第一项是原来的行计算公式， 第二项默认为 show 或者 noshow ，也有为空的时候。 默认为 show k线数据 o c l h v d 等默认空。   增加key => name 项，未来还要增加 线型等 key值  20140222
        if (substr_count($arrtest[$key][0][0], 'my_func')) {
			...
			$my_func_file_name='func/'.$my_func_file[0].'.func.php';  //组成文件名。 
