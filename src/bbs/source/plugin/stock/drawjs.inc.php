			...
			switch ($json[$key][0]['ChartTypes'])
				...
				case 'columnrange':
					...
					for ($x = 1; $x <= $i; $x++){
						//echo strtotime ( $json['datetime'][$x]).'<br>';
						      if ($json[$key][$x] > 0){
						$data_new .='['.strtotime ($json['datetime'][$x]).'000,0,'.round($json[$key][$x], 3).'],';
						}else{
						$data_new1 .='['.strtotime ($json['datetime'][$x]).'000,'.round($json[$key][$x], 3).',0],';
						}
					}
					$data_new1 .= ']},';
					$data_new .= ']},'.$data_new1;
				...
				default:
					...
					for ($x = 1; $x <= $i; $x++){
						//echo strtotime ( $json['datetime'][$x]).'<br>';
						$data_new .='['.strtotime ($json['datetime'][$x]).'000,'.round($json[$key][$x], 3).'],';
					}
					$data_new .= ']},';
				//	echo $data_new ;
